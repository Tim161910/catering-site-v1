...
from.admin_mixins import EventsAdminSiteMixin

class StaffSite(EventsAdminSiteMixin, admin.AdminSite):
    site_header = 'Catering Management Administration'
    site_title = "Catering Portal"
    ...
    def risk_dashboard(self, request):
        today = timezone.now()
        upcoming_events = Event.objects.filter(start_time__gte=today, start_time__lte=today + timedelta(days=14)).prefetch_related('assignments__staff', 'assignments__staff__role').order_by('start_time')
...
# ADMIN CLASSES
class RoleAdmin(admin.ModelAdmin): list_display = ('name', 'slug'); search_fields = ['name']; prepopulated_fields = {'slug': ('name',)}
class EventTemplateRoleInline(admin.TabularInline): model = EventTemplateRole; extra = 1; autocomplete_fields = ['role']
class EventTemplateAdmin(admin.ModelAdmin): list_display = ['name', 'is_active']; list_filter = ['is_active']; search_fields = ['name']; inlines = [EventTemplateRoleInline]; list_editable = ['is_active']
class EventAdmin(admin.ModelAdmin): list_display = ['title', 'start_time', 'client_name']; list_filter = ['start_time']; search_fields = ['title', 'client_name']
class StaffAdmin(admin.ModelAdmin):
    ...
    def reliability_badge(self, obj): score = obj.reliability_score or 0; color = '#28a745' if score >= 90 else '#ffc107' if score >= 75 else '#dc3545'; emoji = '🟢' if score >= 90 else '🟡' if score >= 75 else '🔴'; return format_html('<span style="color: {}; font-weight: 600;">{} {}%</span>', color, emoji, score)
class IssueTypeAdmin(admin.ModelAdmin): list_display = ['name']; search_fields = ['name']
class IncidentAdmin(admin.ModelAdmin): list_display = ['staff', 'event', 'issue_type', 'resolved']; search_fields = ['staff__name']
class AssignmentAdmin(admin.ModelAdmin): list_display = ['event', 'duty_number', 'staff', 'role', 'status']; list_filter = ['status', 'event', 'role']; search_fields = ['staff__name', 'event__title']
